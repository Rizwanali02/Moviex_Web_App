...
const MovieDetail = () => {
  ...
  useEffect(() => {
    const getMovieDetail = async () => {
        ...
        // Fetch related movies based on genres of the selected movie
        const relatedMoviesResponse = await fetch(
          `https://api.themoviedb.org/3/discover/movie?api_key=04c35731a5ee918f014970082a0088b1&with_genres=${data.genres
            .map((genre) => genre.id)
            .join(',')}&exclude=${id}`
        );
